...
Agent-based model (built on [Starsim](https://starsim.org)) that estimates how many **tetanus cases and disease-attributable deaths are averted** when routine pentavalent delivery is scaled up enough to roughly halve the under-5 **zero-dose** share in Kenya.
...
That's it. On first run it calibrates the model to the bundled Kenya HMIS data (`zdsim/data/zerodose_data_formated.xlsx`), saves `calibration.json`, then runs the reference vs scale-up simulations and writes `outputs/`.
...
| `projection_disease_deaths.png` | Yearly disease-attributable deaths from pentavalent modules |
...
2. **Calibration.** `calibrate.py` grid-searches `routine_prob ∈ [0.018, 0.090]` so the simulated end-of-window zero-dose share matches that empirical proxy. The resulting **reference bundle** plus a **scale-up bundle** (higher `routine_prob`, higher `coverage` cap) is saved to `calibration.json`.
3. **Simulation.** `run_simulation.py` builds two `ss.Sim`s from the bundles (20 000 agents, weekly steps, 2025–2055 by default) and runs them back-to-back with a **matched seed** so observed deltas are attributable to the intervention, not RNG draws.
...
- **Vaccine efficacy matches the brief** (`intervention_efficacy = 0.9`); the "50% ZD reduction by 2025" target is not hard-coded — it's what the scale-up bundle is tuned to approach.
