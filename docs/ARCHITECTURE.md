...
├── calibration.json               generated by calibrate.py; required by run_simulation.py
...
`run_simulation.py` builds and runs **three scenarios** from the loaded calibration:
...
`counterfactual` is the validity anchor.
...
> Note: although `household_contacts` is stored on `SimulationParameters`, the runtime `build_simulation` currently attaches only the `community` random-contact network; the household slot is retained for future use and for calibration provenance.

---

## 5. Simulation anatomy (what lives inside `ss.Sim`)

Every scenario is one `ss.Sim` instance composed of these modules.
...
flowchart TB
    subgraph SIM ["ss.Sim · start – stop · dt = 1/52 yr · rand_seed = pars.seed"]
        ...
        subgraph INV ["Interventions (only in baseline + intervention)"]
            I1["ZeroDoseVaccination<br/>coverage · efficacy · routine_prob<br/>age in 0 to 5 yr"]
        end

        subgraph ANA ["Analyzers (when record_yearly=True)"]
...
The **counterfactual** scenario is identical except no `ZeroDoseVaccination` module is attached, so every under-5 is zero-dose by definition (this is a modeling construct, not an empirical claim — the original paper reports a nonzero current zero-dose level).
...
sequenceDiagram
    ...
    loop every timestep ti
        ...
        Sim->>Inv: step — only if ti in timepoints (baseline + intervention)
...
```mermaid
flowchart LR
    SC["sim_counterfactual"] --> C1["cf_zd"]
    SB["sim_baseline"] --> B1["base_zd<br/>zerodose_fraction_under5"]
    SI["sim_intervention"] --> I1["int_zd"]
    SB --> B2["tet_base<br/>tetanus_metrics"]
    SI --> I2["tet_int"]
    SC --> C3["rows_cf"]
...
zero_dose_children_reached          = (base_zd − int_zd) × 7_200_000
